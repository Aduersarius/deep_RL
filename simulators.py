...
class Simulator:
    ...
    def __init__(self, agent, market_emulator, visualizer, log_folder="C:\\Users\\emire\\PycharmProjects\\deep_RL\\logs", enable_epsilon_decay=False):
        self.agent = agent
    ...
    def play_one_episode(self, training=True, rand_price=True, verbose=True):
        ...
        #     action = self.agent.act(state=state, valid_actions=valid_actions)
        #     state, reward, done, valid_actions = self.market_emulator.step(action=action)
        #
        #     # record
        #     self.agent.remember(state, action, reward, None, done, valid_actions)
        #     # train one step (fit)
        #     self.agent.replay()
        #     pass
