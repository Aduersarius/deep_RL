...
class Market:
    ...
    def __init__(self, sampler, window_state, trading_cost, direction=1., risk_averse=0.):
        ...
        self.n_action = 3
        self.state_shape = (window_state, len(self.sampler[0]))
        ...
        self.prices = prices
        self.price = prices / prices[0] * 100
    ...
    def reset(self, rand_price=True):
        self.no_position_flag = True
        if rand_price:
            price = np.reshape(self.prices[:, 0], self.prices.shape[0])

            self.prices = get_data(ticker='SPCE', feed_window=1000, prediction_window=0, size=1, only_price=True)
            self.price = self.prices / self.prices[0] * 100
            # self.t_max = len(self.price) - 1
    ...
    def get_state(self, t=None):
        ...
        for i in range(len(self.sampler[0])):
    ...
    def step(self, action):
        ...
        if action not in self.get_valid_actions():
            raise ValueError(
                'no such action or action is not valid: action={}, valid_actions={}, no_position={}'.format(action,
                                                                                                            self.get_valid_actions(),
                                                                                                            self.no_position_flag))
    ...
    def init_one_frame(self, is_training=True):
...
class MarketEmulator(object):
    ...
    def calc_best_reward(self):
        ...
        if allow_short_sell:
            ...
            best_reward = max(
                np.array(self.action_universe) * np.log(prices_in_reward_period[-1] / prices_in_reward_period[0]))
